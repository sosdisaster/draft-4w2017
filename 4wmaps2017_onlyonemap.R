...
qtm(syrmap, "outpatient_consultations")

# Same code as above, but store the map in a variable:
syrstaticmap <- tm_shape(syrmap) +
  tm_fill("outpatient_consultations", title="Outpatient Consultations ", palette = "YlGnBu") + # PRGn PuBu OrRd YlOrRd  YlGnBu" Greens class 1(no sirve"
  tm_borders(alpha=.5) +
  tm_text("outpatient_consultations", size=0.8) + 
  tm_style_classic()
...
save_tmap(syrstaticmap, filename="outpatient_consultations.jpg")
