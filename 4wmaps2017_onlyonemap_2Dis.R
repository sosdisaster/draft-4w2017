...
##ACA ACA ACA
compiled4w_oc  <- compiled4w[,c("Admin3_Pcode","Admin1_Pcode",	"persons_disabilities")]
...
#ACA ACA ACA
qtm(syrmap, "persons_disabilities")


# Same code as above, but store the map in a variable:
#ACA ACA ACA 
syrstaticmap <- tm_shape(syrmap) +
  tm_fill("persons_disabilities", title="Persons with Disabilities  ", palette = "YlGnBu") + # PRGn PuBu OrRd YlOrRd  YlGnBu" Greens class 1(no sirve"
  tm_borders(alpha=.5) +
  tm_text("persons_disabilities", size=0.8) + 
  tm_style_classic()
...
#ACA ACA ACA
save_tmap(syrstaticmap, filename="persons_disabilities.jpg")
