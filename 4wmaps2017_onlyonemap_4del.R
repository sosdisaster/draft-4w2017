...
##ACA ACA ACA
compiled4w_oc  <- compiled4w[,c("Admin3_Pcode","Admin1_Pcode",	"deliveries")]
...
#ACA ACA ACA
qtm(syrmap, "deliveries")


# Same code as above, but store the map in a variable:
#ACA ACA ACA 
syrstaticmap <- tm_shape(syrmap) +
  tm_fill("deliveries", title="Deliveries  ", palette = "YlGnBu") + # PRGn PuBu OrRd YlOrRd  YlGnBu" Greens class 1(no sirve"
  tm_borders(alpha=.5) +
  tm_text("deliveries", size=0.8) + 
  tm_style_classic()
...
#ACA ACA ACA
save_tmap(syrstaticmap, filename="deliveries.jpg")
